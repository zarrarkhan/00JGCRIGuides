...
getQuery(queryData.proj,"Primary Energy Consumption (Direct Equivalent")   # Example to view a query

params <- c("ag production by tech","Primary Energy Consumption (Direct Equivalent") # Choose your parameters

for (param_i in params){ # Run for each param
# Get Data
df <- getQuery(queryData.proj, param_i)%>%
...
png(paste(param_i,".png",sep=""),width=13,height=9, units="in",res=300)
print(plot)
dev.off()

# Save Table
write.csv(df,file=paste(param_i,".csv",sep=""),row.names=F)
}
```
