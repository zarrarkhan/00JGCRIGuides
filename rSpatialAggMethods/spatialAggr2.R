...
wd0<<-getwd();wd0
wdSpat<-paste(dirname(dirname(wd0)));wdSpat
deleteOldFigs<-1
...
#______________________
#GADM 
gadm36L1<<-readOGR(paste(wdSpat,"/gadm",sep=""),"gadm36_1")
shp_PNNL32Reg<<-readOGR(paste(wdSpat,"/gcam32",sep=""),"region32_0p5deg")
...
m<-tm_shape(shpa1)+tm_borders("black",lwd=3, lty=1)+tm_layout(legend.show=F,frame=F)
pdf(file=paste(wdfigsOut,"/m1a_regionNoNames.pdf",sep=""), height=10, width=8)
m
dev.off()
...
m<-tm_shape(r)+tm_raster(col="X2005",style="kmeans",n=1000,legend.show=F)+tm_layout(frame=F);m
pdf(file=paste(wdfigsOut,"/m2a_polygonRasterRegionGrid.pdf",sep=""), height=10, width=8)
m
dev.off()  
...
tm1<-tm_shape(shpa.x)+tm_fill(col="X2005m1",style="kmeans",n=1000,legend.show=F)+
     tm_borders("black",lwd=3, lty=1)+tm_layout(frame=F);tm1
tm2<-tm_shape(shpa.x)+tm_fill(col="X2005m2",style="pretty",n=10,legend.show=F)+
  tm_borders("black",lwd=3, lty=1)+tm_layout(frame=F);tm2

pdf(file=paste(wdfigsOut,"/ma_sumsm1m2.pdf",sep=""), height=10, width=13)
tmap_arrange(tm1, tm2,asp = NA,ncol=2)
dev.off()
